...
class Game {
    ...
    restoreCardEffects(cards) {
        return cards.map(card => {
            ...
            if (card.suit.type === 'introspection') {
                ...
                effectFn = (s) => {
                    const r = Math.random();
                    const a = (0, utils_1.rand)(10, 20);
                    if (r < 0.33) {
                        if (s.sosLocked)
                            return `ERROR: Antena destruida. SOS fallido.`;
    ...
    initDeck() {
        const suits = [
            {
                s: constants_1.SUITS.HEARTS,
                t: constants_1.CARD_TEMPLATES.HEARTS,
                eFn: (s) => {
                    const h = (0, utils_1.rand)(8, 18); // Aumentado de +5-15% a +8-18%
                    s.mental = Math.min(100, s.mental + h);
                    return `Recuperas ${h}% de Estabilidad Mental.`;
            ...
            {
                s: constants_1.SUITS.DIAMONDS,
                t: constants_1.CARD_TEMPLATES.DIAMONDS,
                eFn: (s) => {
                    const good = Math.random() > 0.4;
                    if (good) {
                        const h = (0, utils_1.rand)(5, 10);
            ...
            {
                s: constants_1.SUITS.CLUBS,
                t: constants_1.CARD_TEMPLATES.CLUBS,
                eFn: (s) => {
                    const d = (0, utils_1.rand)(8, 20);
                    s.mental = Math.max(0, s.mental - d);
                    return `Estabilidad Mental cae ${d}%.`;
            ...
            {
                s: constants_1.SUITS.SPADES,
                t: constants_1.CARD_TEMPLATES.SPADES,
                eFn: (s) => {
                    const r = Math.random();
                    const a = (0, utils_1.rand)(10, 20);
                    if (r < 0.33) {
    ...
    startDay() {
        ...
        if (this.state.adrenalineBoostDays !== undefined && this.state.adrenalineBoostDays > 0) {
            ...
            if (this.state.adrenalineBoostDays === 0) {
                this.onLogEntry?.('> SISTEMA: El efecto de adrenalina ha expirado.', 'system');
            }
        }
        ...
        const dayCards = (0, utils_1.rand)(1, 6);
    ...
    applyDailyPenalties() {
        ...
        if (isAdrenalineActive) {
            this.onLogEntry?.(`> FIN DE DÍA ${this.state.day || 0}: El tiempo pasa. [Adrenalina activa: Penalizaciones anuladas]`, 'cycle-penalty');
        ...
        const baseLifePenalty = (0, utils_1.rand)(1, 2); // -1 a -2% (antes: -1 a -3%)
        const baseMentalPenalty = (0, utils_1.rand)(1, 3); // -1 a -3% (antes: -2 a -4%)
        ...
        this.onLogEntry?.(`> FIN DE DÍA ${day}: El tiempo pasa. Frío: -${lifePenalty}% O₂ | Aislamiento: -${mentalPenalty}% Ψ${penaltyNote}`, 'cycle-penalty');
        // Riesgo de "Suicidio Accidental" si mental < 20
        if (this.state.mental < 20 && Math.random() < 0.1) {
            this.onLogEntry?.(">> LA LOCURA TE CONSUME. Sales a caminar a la tormenta sin traje.", "horror");
            this.finishGame("insanity");
            return;
        }
    }
    ...
    handleDay6Storm() {
        ...
        if (this.state.isAntennaReinforced) {
            this.onLogEntry?.(">> ALERTA DE TORMENTA: Los sensores detectan vientos cataclísmicos.", "horror");
            this.onLogEntry?.("> SISTEMA: La antena reforzada resiste la tormenta. Los tensores aguantan.", "system");
            this.onLogEntry?.("> SISTEMA: Módulo de Transmisión SOS [ONLINE]. La señal se mantiene estable.", "system");
        }
        else {
            // La antena no está reforzada, se bloquea SOS
            this.state.sosLocked = true;
            this.onLogEntry?.(">> ALERTA DE TORMENTA: Los sensores detectan vientos cataclísmicos. La antena de comunicaciones ha sido arrancada de la base.", "horror");
            this.onLogEntry?.("> SISTEMA: Módulo de Transmisión SOS [OFFLINE]. Ya no es posible pedir ayuda externa.", "system");
            this.onLogEntry?.(">> ADVERTENCIA: RANGO DE EXTRACCIÓN EXCEDIDO. El vehículo de rescate ha abandonado el sector.", "horror");
        }
    }
    drawCard() {
        if (this.deck.length === 0) {
        ...
        if (isHearts || (isDiamonds && diamondsRoll > 0.4)) {
            // Intentar dar objeto
            let item = (0, items_1.getItemByCard)(card.suit.symbol, card.rank);
            ...
            if (item) {
                ...
                if (this.cardsDrawnWithoutImportantItem >= 10 && !isImportantItem && isHearts) {
                    ...
                    const specialRank = specialRanks[(0, utils_1.rand)(0, specialRanks.length - 1)];
                    item = (0, items_1.getItemByCard)('♥', specialRank);
                    this.cardsDrawnWithoutImportantItem = 0; // Resetear contador
                    this.onLogEntry?.(`> SISTEMA: [PITY] Objeto importante garantizado después de 10 cartas.`, 'system');
                }
            }
            else {
            ...
            if (item) {
                ...
                if ((0, inventory_1.canAddItem)(this.state.inventory, item, this.state.inventoryMaxSlots)) {
                    const addResult = (0, inventory_1.addItem)(this.state.inventory, item, this.state.inventoryMaxSlots);
                    if (addResult) {
                        ...
                        this.onLogEntry?.(`> ADVERTENCIA: INVENTARIO LLENO. No se puede agregar ${item.name}.`, 'system');
                ...
                const negativeEvent = (0, events_1.generateInfrastructureNegativeEvent)();
            ...
            const anomalyEvent = (0, events_1.generateAnomalyEvent)(this.state.mental);
            ...
            const operationEvent = (0, events_1.generateOperationEvent)();
        ...
        this.onLogEntry?.(`${isThreat ? '>>' : '>'} DATOS ENTRANTES [${card.suit.symbol}${card.rank}]: ${card.text}`, isThreat ? 'horror' : 'system');
        ...
        if (this.state.mental < 40 && !this.showingRealState) {
            const fakeLog = (0, hallucinations_1.checkForFakeLog)(this.state.mental);
        ...
        (0, storage_1.saveGame)(this.state, this.deck, this.allCards);
        if (this.state.mental <= 0) {
            this.finishGame("insanity");
            return;
        }
    ...
    triggerRescue() {
        ...
        this.onLogEntry?.(`> SISTEMA: SEÑAL SOS COMPLETADA. Contacto establecido con base principal.`, "system");
        this.onLogEntry?.(`> SISTEMA: Helicóptero de rescate en camino. Tiempo estimado: ${this.state.rescueTimer} día(s).`, "system");
    }
    /**
     * Verifica si el rescate ha llegado
     */
    checkRescueArrival() {
        this.onLogEntry?.(`> SISTEMA: HELICÓPTERO DE RESCATE DETECTADO. Aterrizando...`, "system");
    ...
    shuffleDeck() {
        ...
        const drawnSpecialCards = this.allCards.filter(card => card.drawn && drawnSpecialRanks.includes(card.rank));
        ...
        this.onLogEntry?.(`> SISTEMA: Mazo barajado. ${this.deck.length} cartas disponibles.`, "system");
    }
    attemptFinish() {
        this.finishGame("manual");
    }
    finishGame(reason) {
        ...
        if (reason === "insanity") {
            ...
            // Si no hay rescate activo pero se finaliza manualmente
                ...
                const endings = [];
                if (this.state.sos >= 80 && !this.state.sosLocked)
                    endings.push("RESCATE");
                if (this.state.firewall >= 80)
                    endings.push("HÉROE");
                if (this.state.analysis >= 80)
                    endings.push("ERUDITO");
                if (endings.length === 3) {
                    title = "EJECUCIÓN PERFECTA";
                    titleColor = "#00ff00";
     ...
     */
    applyMentalModifier(dieRoll, mental) {
     ...
     */
    getMaxRealityChecksPerDay(mental) {
        if (mental > 60) {
            return 2;
        }
        else if (mental > 30) {
    ...
    verifyReality() {
        ...
        if (success) {
            ...
            this.onLogEntry?.(`>> LA ENTIDAD SE RESISTE. La verificación falla.`, 'horror');
            return {
                success: false,
     ...
     */
    processEventChoice(choice, result) {
